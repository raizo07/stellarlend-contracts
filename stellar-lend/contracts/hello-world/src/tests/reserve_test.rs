...
fn test_set_treasury_address_to_contract() {
    ...
    let result = test_set_treasury_address(&env, &contract_id, admin, treasury);
...
fn test_withdraw_reserve_by_non_admin() {
    ...
    let _ = test_withdraw_reserve_funds(&env, &contract_id, user.clone(), asset, 500);
}
...
fn test_reserve_factor_formula_precision() {
    ...
    let test_cases: std::vec::Vec<(i128, i128, i128)> = std::vec![
...
#[test]
fn test_get_reserve_stats_query() {
    let (env, contract_id, admin, _user, treasury) = setup_test_env();
    let asset = Some(Address::generate(&env));
