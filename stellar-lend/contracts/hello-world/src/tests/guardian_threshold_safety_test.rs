...
use crate::types::{GuardianConfig, RecoveryRequest};
...
fn test_guardian_threshold_change_during_recovery_fails() {
    ...
    // Start recovery
    start_recovery(&env, guardian1.clone(), old_admin.clone(), new_admin.clone()).unwrap();
...
fn test_guardian_removal_would_brick_recovery_fails() {
    ...
    let approvals: Vec<Address> = env.storage().persistent().get(&GovernanceDataKey::RecoveryApprovals).unwrap();
...
fn test_recovery_threshold_edge_cases() {
    ...
    start_recovery(&env, guardian1.clone(), Address::generate(&env), Address::generate(&env)).unwrap();
...
fn test_auto_threshold_adjustment_on_removal() {
    ...
    let config: GuardianConfig = env.storage().instance().get(&GovernanceDataKey::GuardianConfig).unwrap();
