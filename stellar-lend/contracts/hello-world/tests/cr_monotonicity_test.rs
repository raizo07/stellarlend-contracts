...
fn setup_env_with_native_asset() -> (Env, Address, HelloContractClient<'static>, Address, Address, Address) {
    let env = Env::default();
    env.mock_all_auths();
...
fn test_repay_monotonicity() {
    ...
    for &debt in debts.iter() {
        for &repay in repays.iter() {
            ...
            let (env, contract_id, client, _admin, user, native_asset) = setup_env_with_native_asset();
            ...
            token_client.approve(&user, &contract_id, &collateral, &(env.ledger().sequence() + 100));
            ...
            if repay > 0 && safe_debt > 0 {
                ...
                token_client.approve(&user, &contract_id, &repay, &(env.ledger().sequence() + 100));
            ...
            assert!(health_after >= health_before, "repay decreased health: debt={:?} repay={:?} before={} after={}", debt, repay, health_before, health_after);
        }
    }
}
...
fn test_borrow_non_improving() {
    ...
    for &collateral in collateral_values.iter() {
        for &borrow in borrows.iter() {
            ...
            assert!(health_after <= health_before, "borrow improved health: collateral={:?} borrow={:?} before={} after={}", collateral, borrow, health_before, health_after);
        }
    }
}
