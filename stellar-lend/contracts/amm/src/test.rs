...
/// Create a protocol config using an explicitly provided address (no contract registration).
/// Use for tests that don't need actual cross-contract swap calls.
fn create_protocol_config_for_addr(env: &Env, protocol_addr: &Address) -> AmmProtocolConfig {
    let mut supported_pairs = Vec::new(env);
    supported_pairs.push_back(TokenPair {
        token_a: None,
...
fn test_swap_failure_zero_amount() {
    ...
    contract.initialize_amm_settings(&admin, &100, &1000, &10000);
    let protocol_config = create_protocol_config_for_addr(&env, &protocol_addr);
    contract.add_amm_protocol(&admin, &protocol_config);
...
fn test_callback_disabled_protocol_fails() {
    ...
    contract.initialize_amm_settings(&admin, &100, &1000, &10000);
    let mut protocol_config = create_protocol_config_for_addr(&env, &protocol_addr);
